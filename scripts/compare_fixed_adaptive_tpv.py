"""Compare one fixed TPV bandgap with temperature-adaptive selection.
...
def main() -> None:
    ...
    with output.open("w", encoding="utf-8") as file:
        file.write("source_temperature_k,adaptive_bandgap_ev,adaptive_sustainable_w_m2,fixed_bandgap_ev,fixed_sustainable_w_m2,gain_w_m2,gain_percent\n")
        for temp in TEMPERATURES:
            candidates = [(sustainable_output(temp, bandgap)[0], bandgap) for bandgap in BANDGAPS]
            adaptive_output, adaptive_bandgap = max(candidates)
            fixed_output, _ = sustainable_output(temp, FIXED_BANDGAP)
